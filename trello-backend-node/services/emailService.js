...
async sendLoginToken(email, token) {
  ...
  // ✅ FIXED: Use the provided email, NOT hardcoded
  const emailData = {
    sender: { 
      name: 'Trello Clone', 
      email: 'charleschmidth@gmail.com' 
    },
    to: [{ email: email }],  // ✅ Use the user's email
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
        ...
        <p style="margin-top: 20px; font-size: 12px; color: #666;">
          Trello Clone | <a href="${process.env.FRONTEND_URL}">${process.env.FRONTEND_URL}</a>
        </p>
      </div>
    `,
    textContent: `Your Trello login verification code is: ${token}\n\nThis code will expire in 10 minutes.\n\nTrello Clone | ${process.env.FRONTEND_URL}`
  };
