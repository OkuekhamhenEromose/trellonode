...
// Reuse the centralized ID normalization helper when validating membership IDs.
const {toIdString} = require('../services/authorizationService')
...
// Get single board. The route middleware has already established membership; the controller focuses on retrieval.
...
exports.createBoard = async (req, res) => {
  try {
    ...
    // Validate the request shape before touching MongoDB.
    ...
    // Normalize and deduplicate requested members; the owner is forced into the set.
    const requestedMemberIds = Array.isArray(member_ids) ? member_ids : [];
    const uniqueMemberIds = [
      ...new Set([
        req.user._id.toString(),
        ...requestedMemberIds.map(toIdString),
      ]),
    ];
    if (uniqueMemberIds.some((id) => !mongoose.isValidObjectId(id))) {
      return res
        .status(400)
        .json({ error: "Invalid member ID", code: "INVALID_MEMBER_ID" });
    }
    // Authorization is not just ID validation: every requested member must correspond
    // to an active user before membership is written to the board.
    ...
    if (users.length !== uniqueMemberIds.length) {
      return res
        .status(400)
        .json({
          error: "One or more members do not exist or are inactive",
          code: "INVALID_MEMBER",
        });
    }
...
exports.updateBoard = async (req, res) => {
  try {
    ...
    if (req.body.background_color !== undefined)
      updates.background_color = req.body.background_color;
    // Membership changes are validated rather than accepting arbitrary ObjectIds.
    if (req.body.member_ids !== undefined) {
      ...
      const uniqueMemberIds = [
        ...new Set([
          req.user._id.toString(),
          ...req.body.member_ids.map(toIdString),
        ]),
      ];
      if (uniqueMemberIds.some((id) => !mongoose.isValidObjectId(id))) {
        return res
    ...
    const updatedBoard = await Board.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true,
    })
      .populate("owner", "username email profile.fullname")
...
// Archive board (soft delete). Ownership is enforced by the route middleware.
